...
const ReactFilters: React.FC<ReactFiltersProps> = ({ posts, categories }) => {
  const [filteredPosts, setFilteredPosts] = useState<CollectionEntry<'posts'>[]>(posts);
      ...
      <div className="filter-buttons flex gap-4 flex-wrap justify-center my-8">
        <button
          className={`px-6 py-2 rounded-md transition-colors ${
            activeCategory === 'all' ? ' bg-[var(--gray-900)] text-[var(--gray-100)]' : 'bg-gray-200 hover:bg-gray-300'
          }`}
        ...
        {categories.map((category) => (
          <button
            key={category}
            className={`px-6 py-2 rounded-md transition-colors ${
              activeCategory === category ? 'bg-[var(--gray-900)] text-[var(--gray-100)]' : 'bg-gray-200 hover:bg-gray-300'
            }`}
        ...
        {filteredPosts.map((post) => (
          ...
          >
            ...
            <div className="p-6">
              ...
              <p className="text-[var(--gray-100)] mb-4">{post.data.description}</p>
